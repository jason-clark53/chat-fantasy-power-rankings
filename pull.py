...
def fetch_team_managers(league_id: int, year: int) -> dict[int, list[str]]:
    """
    Returns {teamId: [manager full names...]}.
    Uses:
      - mTeam.teams[].owners[]  -> owner IDs per team
      - mMembers.members[]      -> owner ID -> firstName + lastName (fallbacks if missing)
    """
    ...
    # 2) Pull member directory (owner ID -> cleaned name)
    ...
    for m in (raw_members.get("members") or []):
        ...
        if first or last:
            ...
            display = m.get("displayName") or m.get("nickname") or oid
        if oid:
            members_map[oid] = display

    # 3) Resolve owners -> names per team
